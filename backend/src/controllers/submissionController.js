...
exports.getStudentSubmissions = asyncHandler(async (req, res, next) => {
  ...
  const submissions = await Grade.find({ student: studentId })
    .populate('assignment', 'title dueDate maxPoints')
    .populate('course', 'title code')
    .sort('-submittedAt');
...
exports.getAssignmentSubmissions = asyncHandler(async (req, res, next) => {
  ...
  const submissions = await Grade.find({ assignment: assignmentId })
...
exports.downloadSubmission = asyncHandler(async (req, res, next) => {
  ...
  if (submission.files && submission.files.length > 0) {
    res.redirect(submission.files[0].fileUrl);
  } else {
    return next(new ErrorResponse('No file available', 404));
  }
});
